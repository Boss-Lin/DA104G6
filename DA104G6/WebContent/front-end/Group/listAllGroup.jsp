...
<%
	GroupService groService = new GroupService();
	List<GroupVO> list = groService.getByStatus();
	pageContext.setAttribute("groList",list);
%>
...
<!------------------------------------------------- 導覽按鈕 -------------------------------------------->
		<div class="col-2 pt-4" id="list_body">
...
<!--------------------------------------------- listAllGroup區塊 --------------------------------------->	
		<div class="col-10">
<!---------------------------------------------- 標題(全部行程...) --------------------------------------->			
			<div class="alert alert-success mt-3 p-2" role="alert" id="title_div">
				<h4 class="alert-heading" id="title_group">全部行程</h4>
			</div>
<!---------------------------------------------- 標題(全部行程...) --------------------------------------->
...
<!------------------------------------------------ 行程瀏覽身體 ------------------------------------------->			
			<div class="row" id="group_list">
				<c:forEach var = "groVO" items = "${groList}" begin = "<%= index %>" end = "<%= index + rowsNum -1 %>" >
				
				<div class="col-xl-3 col-12">
					<div class="card">
						<div class="list_img" style="min-height: 250px; max-height: 250px"><img src="<%= request.getContextPath() %>/front-end/Group/group.do?getImg=${groVO.gro_no}" class="card-img-top" alt="揪團封面圖"></div>
					 	<div class="card-body">
					    	<h3 class="card-title text-center">${groVO.gro_name}</h3>
					    	<p class="card-text text-center text-justify">連絡電話 : ${groVO.phone}</p>
						</div>
							<ul class="list-group list-group-flush">
							    <li class="list-group-item">團主 : <c:forEach var='memvo' items='${msvc.allMem}'>
						...
						<div class="card-body">
							<form class = "submitOne" method= "get">
								<button type="submit" class="btn btn-outline-danger btn-lg" name = "action" value = "getOne_For_Display" formaction = "<%=request.getContextPath() %>/front-end/Group/group.do">觀看詳情</button>
							...
							<c:if test = "${memVO.mem_no ne groVO.mem_no}">
								<form class = "submitOne" method= "get" id="submitjoin" action="<%=request.getContextPath() %>/front-end/Sign_up/sign_up.do">
									...
									<input type = "hidden" name = 'page' value = '<%=whichPage%>'>
									<input type = "hidden" name = 'requestURL' value = '<%=request.getServletPath() %>'>
									<button type="submit" class="btn btn-outline-dark btn-lg" name="action" value="insert" id="join">參團</button>
								</form>
							</c:if>
