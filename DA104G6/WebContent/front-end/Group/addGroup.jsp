...
<head>
    ...
    <link rel="stylesheet" href="<%=request.getContextPath()%>/css/addGroup.css">
...
<jsp:useBean id="Sec" class="com.mem.model.MemService"/>
<%
    ...
    <form action="group.do" method="post" enctype="multipart/form-data">
        <div class="row justify-content-around align-items-center" id="addGroupPage">
            ...
            <div class="col-6 p-5" id="LeftCol">
                ...
                <div class="row justify-content-around align-items-center">
                    ...
                    <div class="col-12 text-center mb-5">
                        ...
                        <h1 class="display-4" style="color: #2F4858">建立行程</h1>
                    ...
                    <div class="col-10 mb-5">
                        <label for="gro_name" id="gro_name">行程名稱 :</label>
                        <c:if test="${not empty errorMsgs}">
                            <font class="adderror">${errorMsgs.get("groNmaeErr")}</font>
                        </c:if>
                        <input type="text" class="form-control in" id="gro_name" placeholder="請輸入行程名稱" maxlength="10" name="gro_name"
                               value="${groVo.gro_name}">
                    </div>
                    ...
                    <div class="col-2 mb-5">
                        <label for="limit">人數上限 :</label>
                        <select class="custom-select in" id="limit" name="peo_limit">
                            <option value="100">不限</option>
                            <% for (int i = 1; i <= 10; i++) { %>
                            <option value="<%= i %>" <%= groVo != null ? (i == groVo.getPeo_limit() ? "selected" : "") : ""%>><%= i %></option>
                            <% } %>
                        </select>
                    </div>
                    ...
                    <div class="col-12 mb-5">
                        ...
                        <label for="address">集合地點 :</label>
                        <c:if test="${not empty errorMsgs}">
                            <font class="adderror">${errorMsgs.get("musterErr")}</font>
                        </c:if>
                        <div id="twzipcode"></div>
                        <input type="text" class="form-control in" id="address"
                               placeholder="例: 屏東縣枋山鄉金龍二街六段707巷395弄451號" maxlength="30" name="muster" value="${groVo.muster}">
                    </div>
                    ...
                    <div class="col-6 mb-5">
                        <label for="days">活動天數 :<output id="choice">${empty groVo.duration ? 2 : groVo.duration}</output>天</label>
                        <input type="range" class="custom-range" min="1" max="20" step="1" id="days" value='${empty groVo.duration ? 2 : groVo.duration}' oninput="choice.value = parseInt(days.value)" name="duration">
                    </div>
                    ...
                    <div class="col-6 mb-5">
                        ...
                        <c:if test="${not empty errorMsgs}">
                            <font class="adderror">${errorMsgs.get("timeErr")}</font>
                        </c:if>
                    ...
                    <div class="col-6 mb-5">
                        <label for="deadline">報名截止日 :</label>
                        <c:if test="${not empty errorMsgs}">
                            <font class="adderror">${errorMsgs.get("deadlineErr")}</font>
                        </c:if>
                    ...
                    <div class="col-6 mb-3">
                        <label class="in" for="phone">連絡電話 :</label>
                        <c:if test="${not empty errorMsgs}">
                            <font class="adderror">${errorMsgs.get("phoneErr")}</font>
                        </c:if>
                        <div class="input-group mb-3">
                            <input type="text" class="form-control in" id="phone" name="phone" value="${groVo.phone}"
                    ...
                    <div class="col-12">
                        <label for="intro">行程簡介 :</label>
                        <textarea class="form-control is-valid info-container" id="intro"
                                  name="intro">${groVo.intro}</textarea>
            ...
            <div class="col-12 mt-5 mb-5 text-center">
                ...
                <button type="submit" class="btn btn-primary" name="action" value="insert">建立行程</button>
                <a class="btn btn-info" href="<%=request.getContextPath()%>/front-end/Group/listAllGroup.jsp" role="button">瀏覽行程</a>
            </div>
...
//	3.以下為兩個日期之外的日期無法選擇 (也可按需要換成其他日期)
    ...
    $("#time").change(function () {
    	if($("#time").val() != ''){ $('#deadline').attr('readonly', false);}
        let dead = $("#time").val().substring(0, 10)
        $('#deadline').val('');
        somedate2 = new Date(dead);
        somedate2.setDate(somedate2.getDate() - 3);
    });
