<%@page import="com.route_category.model.Route_CategoryVO"%>
<%@page import="com.route_category.model.Route_CategoryService"%>
<%@ page contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>
<%@ page import="java.util.*"%>
<%@ page import="com.route.model.*"%>
<%
List<RouteVO> list = (List<RouteVO>)request.getAttribute("routelist");
if(list == null){
	RouteService routeSvc = new RouteService();
	list = routeSvc.getAll();
}
...
<body>
   <jsp:include page="/front-end/include/NavBar.jsp" flush="true" />
    ...
    <div class="container-fluid">
        ...
        <div class="row">
            ...
            <div class="col-12 col-md-3">
                <nav class="routenavbar navbar-light bg-transparent">
                    <form class="form-inline" METHOD="post" ACTION="<%=request.getContextPath()%>/route/route.do" name="form1">
                        <input class="form-control" type="search" name="keyword" placeholder="輸入路線名稱關鍵字..." aria-label="Search">
                        <button class="btn btn-outline-success" type="submit" name="action" value="search">搜尋</button>
                    </form>
                </nav>
            </div>
        </div>
            ...
            <div class="col-4 col-md-2 text-center" id="RouteCategory">
                <div class="list-group">
                    ...
                    <a href="<%=request.getContextPath()%>/route_category/route_category.do?route_cate_no=RC005&action=popularCate"
                     class="list-group-item list-group-item-action " id="btnpopular"><i class="fab fa-hotjar"></i> 熱門路線</a> 
    ...
    <c:if test="${not empty addRouteSuccess}">
	    swal("建立成功", "您的路線已建立!", "success");
		<%request.removeAttribute("addRouteSuccess");%>
    </c:if>
	
    
    
    <c:if test="${empty list}">
		swal("無資料", "目前還沒有路線相關資料!", "warning");
		<%request.removeAttribute("list");%>
	</c:if>
									

	<c:if test="${not empty nullsearch}">
		swal("搜尋不得為空", "請確實輸入關鍵字!", "error");
		<%request.removeAttribute("nullsearch");%>
	</c:if>
